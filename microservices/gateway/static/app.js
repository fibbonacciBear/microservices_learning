...
async function loadKitchenQueue() {
  try {
    ...
    queue.forEach((entry) => {
      ...
      const disabled = entry.status === "done" ? "disabled" : "";
      item.innerHTML = `
        <div>
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          showMessage(`Order #${entry.order_id} cooked successfully.`);
          await Promise.all([loadKitchenQueue(), loadOrders(), loadNotifications()]);
        } catch (error) {
          showMessage(error.message, true);
          await Promise.all([loadKitchenQueue(), loadOrders()]);
        } finally {
...
async function loadNotifications() {
  ...
  try {
    ...
    feed.innerHTML = `
      <div class="card">
        <strong>Notification service unavailable</strong>
        <p class="muted">Orders and cooking still work, but notification history is temporarily unavailable.</p>
      </div>
    `;
  }
}
